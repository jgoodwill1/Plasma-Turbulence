...
// Whether to use Kelvin Helmholtz Instability
#define KHI
...
begin_initialization {
  ...
  int particle_select = 330000; // track one every particle_select particles
  ...
  double L_di = 0.1;            // Half thickness of the current sheet

  double betai = 0.05;
  double betae = 0.05;
  double beta = betai + betae;
  double theta   = 2.86;   // B0 = Bx
  ...
  double vthe    = sqrt(betae* Bn*Bn/n0);   // Electron thermal speed over c
  ...
  double taui    = 50.0; // simulation wci's to run
...
//   double topology_z = 1; 
  ...
  double v_A_ang   = Bn * sn /sqrt(n0 * me * mi_me);    
  


  double U0 = 10.0 * v_A_ang;
  double Uz0 = -U0*sn;
  double Uy0 =  U0*cs;
  ...
  double dV0 = 0.15 ; // amplitude of the initial pertubation
    
  //Excess Charge
  double extra_frac = 2.0*(Uy0*Bz0-Uz0*By0)/Lx*tanh(Lx/(2.0*L_di));

  double extra_electrons = ( Ne * extra_frac);
#endif
  ...
  double dbz = 0.02*Bn;                    // Perturbation in Bz rel. to Bo (Only change here)
  ...
  int restart_interval = int(1000000.0/(wpe*dt));
  ...
  int nx_zone = nx/topology_x;             // # of cells / zone along x for energy spectrum diagnostics
                                // MAKE SURE that (nx / topology_x) is divisible by nx_zone
  int ny_zone = ny/topology_y;             // # of cells / zone along y for energy spectrum diagnostics
                                // MAKE SURE that (ny / topology_y) is divisible by ny_zone
  int nz_zone = nz/topology_z;           // # of cells / zone along z for energy spectrum diagnostics
                                // MAKE SURE that (nz / topology_z) is divisible by nz_zone

  // particle dump
  int stride_particle_dump = 40; // stride for particle dump

  // Determine which domains area along the boundarie  Us
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                 \
  ...
  double x, y, z, ux, uy, uz;
    //      double x = uniform_rand( 0, Lx ); 
    //      double y = uniform_rand( 0, Ly );
    //      double z = uniform_rand( 0, Lz ); 
...
//  Reflect particles on X and Z boundaries 
  ...
  if (rank() == 0 ) {
    ...
    fp_info.print( " betai = %e\n", betai);
    fp_info.print( " betae = %e\n", betae);
...
#ifdef KHI
    fp_info.print( " xmin = %e\n", xmin);
    fp_info.print( " xmax = %e\n", xmax);
    fp_info.print( " B0 = %e\n", Bn);
    fp_info.print( " U0 = %e\n", U0);
    fp_info.print( " L = %e\n", L);
    fp_info.print( " tanhx = %e\n", tanh(0));
    fp_info.print( " dx = %e\n", hx);
    fp_info.print( " xcen/L = %e\n", xcenter/L); 
#endif
    fp_info.print( " qi = %e\n", qi );
    fp_info.print( " qe = %e\n", qe );
  ...
  //Kelvin Helmholtz setup: Karimabadi et. al. 2013

  #define BX 0
  #define BY (By0)
  #define BZ (Bz0)
  #define EX (-(Bn * U0) * tanh((x - xcenter)/L))
  
  set_region_field(everywhere,EX,0,0,BX,BY,BZ);
...
// LOAD PARTICLES
  ...
  double kKH = 0.4/L_di;   // wavelength of the KH from linear theory

  sim_log( "Loading particles" ); 

  // seed_rand( rng_seed*nproc()+rank() );                          // WARNING: Generators are de-synchronized

  float loaded_fraction = 0.1;

  int np_loaded=0;

  while (np_loaded<Ne/nproc()) {
	...
	// if ((x-xcenter)>0) {
	// electron = electronTop;
	// ion = ionTop;
	// } 
	// else
	// {
	// electron = electronBot;
	// ion = ionBot;
	// }
    
    double vx = normal(rng(0), 0, vthe) + dux; 
    double vy = normal(rng(0), 0, vthe) + duy;
    double vz = normal(rng(0), 0, vthe) + duz;
    ...
    inject_particle(electron, x, y, z, vx, vy ,vz, weight,0,0 );
    
    
    // dux = 0;
    // duy = U0 * tanh((x - xcenter)/L_di);
    // duz = 0;

    dux = 0;
    duy = U0* tanh(((x-xcenter)/L)); 
        // + dV0*Uy0*sin(kKH*y)*exp(-(x-xcenter)*(x-xcenter)/(L * L));
    duz = 0;
      
    vx = normal(rng(0), 0, vthi) + dux;
    vy = normal(rng(0), 0, vthi) + duy;
    vz = normal(rng(0), 0, vthi) + duz;
    ...
    inject_particle( ion, x, y, z, vx, vy,vz, weight ,0,0);
    ++iparticle;
    // put np_loaded inside the "if" to load NPIC-style, i.e. fixed number of particles
    // leaving it here loads correct density
    
    np_loaded++;
    
    if (np_loaded > loaded_fraction*Ne/nproc()) {
      sim_log("loaded "<<loaded_fraction<<" of particles");	
      loaded_fraction = loaded_fraction+0.1;
    }
  }
  
  
  // load the extra electrons

  sim_log( "Loading extra electrons" ); 
  
  np_loaded=0;
  
  double extra_den_edge = tanh(Lx/(2.0*L_di));
  
  
  for (int i=0; i<extra_electrons; i++) {
    
    double r = uniform(rng(0), 0, 1 ); 
    double y = uniform(rng(0), 0, Ly );
    double z = uniform(rng(0), 0, Lz ); 
    
    
    double x = xcenter + L_di*atanh(extra_den_edge*2.0*(r-0.5));
